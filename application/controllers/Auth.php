...
class Auth extends CI_Controller
{
	...
	private function _login()
	{
		...
		if ($user) {
			...
			$querycafe = $this->db->get_where('cafe', ['email_pemilik' => $email])->row_array();
			if($querycafe){
				if($querycafe['status'] == 1){
					$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">cafe Belum Diaktivasi</div>');
					redirect('auth');
				}else if($querycafe['status'] == 2){
					if (password_verify($password, $querycafe['password'])) {
						$data  = [
							'email' => $querycafe['email_pemilik'],
							'id_cafe' => $querycafe['id_cafe'],
							'nama' => $querycafe['nama_ketua'],
							'nama_cafe' => $querycafe['nama_cafe']
						];
						$this->session->set_userdata($data);
						redirect('pengelola');
					}else{
					...
					$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">cafe belum terdaftar</div>');
					redirect('auth');
				}
			}else{
	...
	public function verify()
	{
		...
		$user = $this->db->get_where('cafe', ['email_pemilik' => $email])->row_array();
	...
	public function logout()
	{
		...
		$this->session->unset_userdata('nama_cafe');
		$this->session->unset_userdata('id_cafe');
